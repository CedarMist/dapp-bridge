...
contract WrappedROSE is ERC20, ERC20FlashMint, ERC20Burnable, Ownable {
    ...
    function mint(address in_holder, uint in_amount) external onlyOwner {
        _mint(in_holder, in_amount);
    }

    function _onBurn(uint in_amount, address in_receiver) internal {
        if( in_amount > 0 ) {
            IBridgeInterface(owner()).burn(in_receiver, in_amount);
        }
    }

    function burn(uint in_amount) public override {
        _onBurn(in_amount, _msgSender());
        _burn(_msgSender(), in_amount);
    }

    function burn(uint in_amount, address in_receiver) public {
    ...
    function burnFrom(address account, uint256 value, address in_receiver) public {
